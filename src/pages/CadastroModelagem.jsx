  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px', animation: 'fadeInDown 0.5s ease-out' }}>
        ...
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: '600', color: 'var(--text-primary)' }}>Cadastrar nova modelagem</h1>
      ...
      <section className="settings-container" style={{ display: 'block' }}>
        <div className="settings-card" style={{ maxWidth: '700px' }}>
          <form className="settings-form" onSubmit={(e) => { e.preventDefault(); alert('Modelagem salva com sucesso!'); navigate(-1); }}>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label>Tipo de molde *</label>
              <input type="text" placeholder="Ex: Camiseta Básica, Calça Skinny" required />
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label>Tamanho</label>
                <input type="text" placeholder="Ex: P, M, G, GG ou 36 a 44" required />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <label>Código do Molde</label>
                <input type="text" placeholder="Ex: MOD-001" required />
              </div>
            </div>
